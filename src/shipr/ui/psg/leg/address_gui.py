...
from shipr.models.address import Contact
...
def contact_frame(contact: Contact):
    layout = [
        [sg.Text('Name:', **address_fieldname_params),
         sg.InputText(f'{contact.name}', key='-NAME-', **address_input_params)],

        [sg.Text('Email:', **address_fieldname_params),
         sg.InputText(f'{contact.email}', key='-EMAIL-', **address_input_params)],

        [sg.Text('Telephone:', **address_fieldname_params),
         sg.InputText(f'{contact.telephone}', key='-TELEPHONE-', **address_input_params)],
    ]
    return sg.Frame("Contact Details", layout=layout)


def address_from_postcode_popup(candidate_keys: list[AddressKey],
                                client: DespatchBaySDK) -> Address:
    ...
    layout = [[sg.Listbox(list(candidate_dict.keys()), select_mode=sg.LISTBOX_SELECT_MODE_SINGLE,
                          expand_x=True,
                          expand_y=True, enable_events=True, size=(50, 20), bind_return_key=True,
                          default_values=candidate_keys[0].address, k='select',
                          auto_size_text=True)]]
...
def address_from_gui(shipment: ShipmentInput, address: Address, contact: Contact,
                     client: DespatchBaySDK) -> Address | None:
...
def address_postcode_click(postcode: str, client,
                           address_popup=address_from_postcode_popup) -> Address | None:
